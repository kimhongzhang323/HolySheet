...
class ArcballControl {
    ...
    // Auto-rotate kick logic
    private autoRotateTimer = 0;
    private autoRotateDelay = 4000; // ms to pause between turns
    ...
    public update(deltaTime: number, targetFrameDuration = 16): void {
        ...
        if (!this.isPointerDown && this.autoRotate) {
            ...
            if (this.autoRotateTimer > this.autoRotateDelay) {
                this.autoRotateTimer = 0;

                // Generate random direction (Up/Down/Left/Right)
                // We apply a "kick" to the pointerRotation to simulate a user swipe
                const axis = vec3.create();
                vec3.random(axis);

                // Bias towards horizontal but allow vertical
                axis[1] *= 0.5;
                vec3.normalize(axis, axis);

                const kickAngle = (Math.PI / 2) * this.autoRotateSpeed; // 90 degree turn roughly

                // Apply kick
                const kickRot = quat.create();
                quat.setAxisAngle(kickRot, axis, kickAngle);
                quat.multiply(this.pointerRotation, kickRot, this.pointerRotation);
            }
        } else {
            this.autoRotateTimer = 0;
        }
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
    ...
    const [autoTurn, setAutoTurn] = useState<boolean>(false);
    ...
    useEffect(() => {
        if (sketchRef.current) {
            sketchRef.current.autoRotate = autoTurn;
        }
    }, [autoTurn]);

    const handleButtonClick = () => {
        if (!activeItem?.title) return;
        const searchUrl = `https://www.google.com/search?q=${encodeURIComponent(activeItem.title)}`;
    ...
    return (
        <div className="relative w-full h-full group">
            <canvas
                ...
                ref={canvasRef}
                className="cursor-grab w-full h-full overflow-hidden relative outline-none active:cursor-grabbing block touch-none"
            />

            {/* Auto Turn Toggle */}
            <div className="absolute top-6 right-6 z-30 pointer-events-auto">
                <button
                    onClick={() => setAutoTurn(!autoTurn)}
                    className={`
                        flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all shadow-sm
                        ${autoTurn
                            ? 'bg-emerald-500 text-white shadow-emerald-500/30 ring-2 ring-emerald-200'
                            : 'bg-white/90 text-gray-500 hover:bg-white hover:text-gray-900 border border-gray-200'}
                    `}
                >
                    <div className={`w-2 h-2 rounded-full transition-colors ${autoTurn ? 'bg-white animate-pulse' : 'bg-gray-400'}`} />
